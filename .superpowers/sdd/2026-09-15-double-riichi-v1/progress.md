...
Task 14: complete at 6c04978 — real Axum `/mcp` plus authenticated `double_riichi_mcp` duplex stdio bridge drives one MCP player and built-in opponents through a bounded resource/tool Match; exact Tools/Templates/Resources, subscriptions, privacy, monotonic revisions, wait/action IDs, terminal `game_ended`/Post-Match result, and shutdown/auth coverage pass. Production binary still reads `DRIICHI_MCP_TOKEN` exclusively; token injection is non-default `test-support` only for the in-process bridge test. Parent verification passed fmt, MCP/server focused tests, missing-token CLI test, workspace check, and all workspace tests. Focused MCP Clippy passed; strict server/workspace Clippy remains blocked by documented pre-existing core/replay/server baseline lints. Review package: `review-e2b1b79..HEAD.diff`.
