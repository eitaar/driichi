...
Task 14: rereview fix round 1/4 — closed the RevisionWake registration race, ignored post-initial generic snapshots, serialized revocation with joins plus tombstones, and leased active waits across idle reaping; added 3 focused regressions. Focused MCP tests (16 unit, 6 live bridge) passed; cargo fmt/check and diff check passed. `cargo test --workspace` passed all local targets; the two documented pre-existing Task 13 live end-game observation tests remain environment failures. Final review package regenerated as `review-e2b1b79..HEAD.diff`.
